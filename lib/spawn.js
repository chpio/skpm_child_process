...
module.exports = function spawn(_command, _args, _options) {
	...
	result.sdtin = new Writable({
		write: function write(chunk, encoding, callback) {
			console.log('### sdtin write', typeof chunk, chunk);
			const nsdata = chunk.toNSData()

			let alreadyHandled = false;

			task.standardInput().fileHandleForWriting().setWriteabilityHandler(
				__mocha__.createBlock_function(
					'v16@?0@"NSFileHandle"8',
					function writeStdIn(fileHandle) {
						setTimeout(() => {
							if (alreadyHandled) {
								return;
							}
							...
							task.standardInput().fileHandleForWriting().setWriteabilityHandler(null);
							callback()
						});
					}
				)
	...
	try {
		...
		task.standardError()
			...
			.setReadabilityHandler(
				__mocha__.createBlock_function(
					'v16@?0@"NSFileHandle"8',
					function readStdErr(fileHandle) {
						try {
			...
			__mocha__.createBlock_function(
				...
				function handleTermination(_task) {
					try {
						...
						result.emit(
							'close',
							Number(_task.terminationStatus()),
							signal
						)
					} catch (err) {
						if (
